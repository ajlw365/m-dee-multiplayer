...
> = {
  darkblue: {
    bg: "bg-card-darkblue-bg",
    border: "border-card-darkblue-border",
    text: "text-card-darkblue",
    accent: "bg-card-darkblue",
    banner: "bg-card-darkblue-banner",
  },
  lightblue: {
    bg: "bg-card-lightblue-bg",
    border: "border-card-lightblue-border",
    text: "text-card-lightblue",
    accent: "bg-card-lightblue",
    banner: "bg-card-lightblue-banner",
  },
  brown: {
    bg: "bg-card-brown-bg",
    border: "border-card-brown-border",
    text: "text-card-brown",
    accent: "bg-card-brown",
    banner: "bg-card-brown-banner",
  },
  green: {
    bg: "bg-card-green-bg",
    ...
    banner: "bg-card-green-banner",
  },
  utility: {
    bg: "bg-card-utility-bg",
    border: "border-card-utility-border",
    text: "text-card-utility",
    accent: "bg-card-utility",
  ...
  orange: {
    ...
    border: "border-card-orange-border",
  ...
  pink: {
    bg: "bg-card-pink-bg",
    border: "border-card-pink-border",
    text: "text-card-pink",
    accent: "bg-card-pink",
    banner: "bg-card-pink-banner",
  },
  railroad: {
    bg: "bg-card-railroad-bg",
    border: "border-card-railroad-border",
    text: "text-card-railroad",
    accent: "bg-card-railroad",
    banner: "bg-card-railroad-banner",
  },
  purple: {
    bg: "bg-card-purple-bg",
    border: "border-card-purple-border",
...
function actionBg(name: string): string {
  ...
  if (n.includes("BIRTHDAY") || n.includes("DEBT"))
    return "bg-[oklch(0.38_0.22_15)]";
...
export function GameCard({
  ...
  if (isPropertyCard(card)) {
    ...
    return (
      ...
      >
        <div
          className={cn(
            "relative flex items-center justify-center px-1.5",
            s.bannerH,
            c.banner,
          )}
        >
          <span
            className={cn(
              "font-black uppercase text-center leading-tight text-white drop-shadow-sm",
              s.nameText,
            )}
          >
            {card.name}
        ...
        <div
          className={cn(
            ...
            size === "md"
              ? "h-6 w-6 text-[8px] top-[20px]"
              : "h-3.5 w-3.5 text-[5px] top-[9px]",
          )}
        >
          {card.value}M
        </div>
        <div
          className={cn(
            ...
            size === "md" ? "px-1.5 pt-3 pb-1" : "px-0.5 pt-1 pb-0.5",
          )}
        >
          {size === "md" && (
            <div className="flex flex-1 flex-col rounded-lg border border-[oklch(0.85_0.01_260)] bg-[oklch(0.97_0.003_260)] px-2 py-1">
              <p className="text-[7px] font-bold uppercase tracking-wider text-[oklch(0.40_0.01_260)] mb-0.5 text-center">
                Rent
              </p>
              <div className="flex flex-col gap-px flex-1 justify-center">
                {card.rentTable.map((r) => (
                  ...
                  >
                    <span className="text-[7px] text-[oklch(0.35_0.01_260)]">
                      {r.cards} Card{r.cards > 1 ? "s" : ""}
                    </span>
                    <span className="text-[8px] font-black text-[oklch(0.20_0.01_260)]">
                      ${r.rent}M
                    </span>
                  </div>
                ))}
              </div>
              <p className="text-[6px] text-center text-muted-foreground mt-0.5">
                {card.setRequired} to complete
              </p>
            </div>
          )}
        </div>
    ...
    return (
      ...
      >
        ...
        <DollarSign
          className={cn(
            "mb-1",
            isWild ? "text-white" : "text-[oklch(0.30_0.02_260)]",
            size === "md" ? "h-5 w-5" : "h-2.5 w-2.5",
          )}
          aria-hidden="true"
        />
        <span
        ...
        {size === "md" && isWild && (
          <p
            className="text-[7px] text-center text-white mt-1 px-2"
    ...
    if (!isMulti && Array.isArray(card.colors) && card.colors.length === 2) {
        ...
        return (
          ...
          >
            ...
            <div
              ...
              style={{ height: "24%" }}
            >
              <span className="text-[7px] font-black uppercase text-white drop-shadow-sm text-center leading-tight">
                WILD PROPERTY
              </span>
            </div>

            {/* CENTER WHITE SECTION */}
            <div className="flex-1 relative flex flex-col items-center justify-center px-1 py-1">
              {/* Value circle - positioned halfway between color bar and center box */}
              <div
                className={cn(
                  "absolute left-1/2 -translate-x-1/2 flex items-center justify-center rounded-full bg-white font-black border border-[oklch(0.20_0.01_260)] z-10",
                  size === "md"
                    ? "h-6 w-6 text-[8px] top-1"
                    : "h-3.5 w-3.5 text-[5px]",
                )}
              >
                {card.value}M
              </div>
              ...
              <div className="flex-1 w-full flex flex-col rounded-md bg-slate-100 border border-slate-300 px-0.5 pt-2 pb-2 overflow-hidden mb-0">
                <p className="text-[7px] font-bold uppercase text-slate-700 mb-1">
                  Rent
                </p>

                {/* Both rent tables displayed inline */}
                <div className="flex justify-between items-start text-[5px]">
                  {/* Top color rent table - left side */}
                  <div className="flex justify-center gap-1 flex-1">
                    {/* Cards column */}
                    <div className="flex flex-col items-center">
                      <p className="text-[5px] font-bold text-slate-600 mb-0.55">
                        Cards
                      </p>
                      {topRentTable.map((r) => (
                        <p
                    ...
                    {/* Rent column */}
                    <div className="flex flex-col items-center">
                      <p className="text-[5px] font-bold text-slate-600 mb-0.0">
                        Rent
                      </p>
                      {topRentTable.map((r) => (
                        <p
                          key={`top-rent-${r.cards}`}
                  ...
                  <div className="flex justify-center gap-1 flex-1 rotate-180">
                    {/* Cards column */}
                    <div className="flex flex-col items-center">
                      <p className="text-[5px] font-bold text-slate-600 mb-0.50">
                        Cards
                      </p>
            ...
            <div
              className={cn(
                "flex items-center justify-center flex-shrink-0 rotate-180",
                bottomMap.banner,
              )}
              style={{ height: "24%" }}
            >
      ...
      return (
        ...
        >
          ...
          >
            <span className="text-[4px] font-black uppercase text-white drop-shadow-sm">
              WILD
            </span>
          </div>
          <div
            className={cn(
              "absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center rounded-full bg-white font-black border border-[oklch(0.20_0.01_260)] z-10",
              "h-3.5 w-3.5 text-[5px]",
            )}
          >
            M{card.value}
    ...
    return (
      ...
      >
        <Shuffle
          className={cn(
            "mb-1",
            isMulti ? "text-white" : "text-[oklch(0.30_0.02_260)]",
            size === "md" ? "h-5 w-5" : "h-2.5 w-2.5",
          )}
          aria-hidden="true"
        />
  ...
  if (isActionCard(card)) {
    ...
    return (
      ...
      >
        ...
        {size === "md" && (
          <p
            className="text-[7px] text-center leading-snug text-white px-2 mt-1.5 max-w-full"
            style={{ textShadow: "0 1px 2px rgba(0,0,0,0.3)" }}
          >
            {card.description}
          </p>
  ...
  if (isMoneyCard(card)) {
    ...
    return (
      ...
      >
        <Banknote
          className={cn("mb-1", size === "md" ? "h-5 w-5" : "h-2.5 w-2.5")}
          aria-hidden="true"
        />
        <span
          className={cn(
            "font-black",
            size === "md" ? "text-2xl" : "text-[8px]",
          )}
        >
          {card.value}M
        </span>
        {size === "md" && (
          <span className="text-[7px] font-bold uppercase tracking-wider mt-0.5 opacity-70">
            Bank It
          </span>
        )}
      </button>
